#!/usr/bin/env python
"""
Launch ERA5 2D surface variable processing to HEALPix grid.

This script handles ERA5 2D surface variables which are stored in monthly files
within monthly directories (different from 3D pressure level variables).
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='Process ERA5 2D surface variables to HEALPix grid',
...
Examples:
  ...
  # Time subsetting: Edit time_subset in config file (e.g., time_subset: '3h')
    ...
    parser.add_argument('-c', '--config', type=str,
                       default='../config/era5_2d_config.yaml',
                       help='Config file path (default: config/era5_2d_config.yaml)')
    parser.add_argument('--vars', nargs='+', type=str,
                       help='Variables to process (default: all variables in config). '
                            'Use ERA5 variable names (e.g., TCWV, CAPE, SSTK)')
...
def main():
    ...
    print(f"ERA5 2D Surface Variable Processing")
    print(f"===================================")
    ...
    # Search for files (monthly_files=True for 2D surface variables)
    # Pass variables list directly (not converted to dict)
    input_files = get_era5_input_files(
        start_date=start_date,
        end_date=end_date,
        base_dir=base_dir,
        variables=variables,  # Pass list of dicts directly
        file_template=file_template,
        monthly_files=True  # Key difference from 3D processing
    )
    ...
    print("\n" + "="*80)
    print("Starting ERA5 2D to HEALPix processing...")
    print("="*80 + "\n")
    
    # Run processing (same pattern as 3D launcher)
    try:
