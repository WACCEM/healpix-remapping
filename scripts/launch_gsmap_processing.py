...
Usage: 
    python launch_gsmap_processing.py START_DATE END_DATE [options]

Examples:
    python launch_gsmap_processing.py 2020-01-01 2020-12-31 -z 9
    python launch_gsmap_processing.py 2020-01-01 2020-01-31 -z 9 --overwrite
    python launch_gsmap_processing.py 2020-01-01 2020-12-31 -c custom_config.yaml -z 9

Configuration:
    Edit config/gsmap_config.yaml (or specify custom config) to configure:
    - Input/output paths
...
Date formats:
  ...
  YYYY-MM-DDTHH    - Date with hour (end_date extends to HH:59:59)
    ...
    parser.add_argument('-c', '--config', type=str, default=None,
                        help='Path to config YAML file (default: config/gsmap_config.yaml)')
    ...
    parser.add_argument('--region-store', type=str, default=None,
                        help='Write into a time-region of this PRE-EXISTING HEALPix Zarr store '
                             '(created by scripts/init_healpix_store.py), instead of creating a '
                             'new standalone output file. For parallel multi-task processing '
                             '(e.g. one SLURM array task per year) into a single shared store. '
                             'When set, --output/--overwrite are ignored.')
...
def main():
    ...
    if args.config:
        ...
        config_path = script_dir.parent / "config" / "gsmap_config.yaml"
    ...
    # Region-store mode: write into a time-region of a pre-existing shared
    # store instead of a new standalone one (see scripts/init_healpix_store.py).
    # Overrides --output/--overwrite, which don't apply to region writes.
    region_store = args.region_store
    if region_store:
    ...
    if region_store:
        print(f"\n🧩 Region-store mode: writing into existing store: {region_store}")
        if overwrite:
            print("   (--overwrite is ignored in region-store mode; a region write always replaces its own target region)")
    else:
    ...
    if region_store:
        print(f"Data written to region of: {region_store}")
    else:
